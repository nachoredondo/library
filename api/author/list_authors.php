...
class SSPAuthor extends SSP {
	private const FROM = "`author` as a ";
	private const FROM_LEFT_JOIN = "
					LEFT JOIN `book_authors` as ba ON a.id = ba.id_author  
					LEFT JOIN `book` as b ON ba.id_book = b.id";
	...
	private static function do_search($request, $conn, $columns) {
		...
		if (isset($request['assigned'])) {
			...
			if ($request['assigned'] == "true") {
				...
				$sql_info = "SELECT COUNT(a.`id`)
					FROM $from
					WHERE $where_join";
			} else {
				$where = self::where_add($where, " `a`.`id` ");
				$where_assigned = self::where_assigned($request, $bindings);
				
				$sql_data = "SELECT `a`.*
					 FROM author as a
					$where NOT IN (SELECT a.`id`
						FROM $from
...
echo json_encode(
	SSPAuthor::exec($_POST, get_db_connection())
);
